...
https://www.wxwidgets.org/downloads/. For the Linux build use the package
manager to install the wx dependencies. For WSL2 this should be wx-common and
libwxgtk3.0-gtk3-dev. There is no Intel-specific SIMD code so in theory
...
(dandelions_setup.msi). For the Linux build, clone the Git repository,
cd into it, run make, and copy the executable bin/dandelions to a
...
(https://wiki.wxwidgets.org/) for it's UI. The contrasting palette of
